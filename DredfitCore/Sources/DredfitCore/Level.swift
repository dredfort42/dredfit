...
public enum Level {
    public static func decode(_ level: Int) -> LevelDecoded {
        ...
        // Inside a sets band the start and the step are the band's own —
        // otherwise entering a band halves the dose.
    ...
    /// inversion off at a tier's edge is not an option. Measured while
    ...
    /// Where a pain report lands — the bottom of the previous tier. A change
    /// of variation, not fewer reps of the same one: "take the load off, don't
    /// trim it". The set bands are tier 4 by encoding, so they too land at the
    /// bottom of tier 3.
    ...
    /// The floor of the CURRENT tier — the first step of taking the load off.
    /// The variation does not change; the dose becomes the smallest that
    /// variation has. The work falls by construction: `repStart` is the tier's
    /// minimum and the mod-8 rung goes to zero. Swept over 10 patterns × 48
    /// levels: 0 cells where the landing asks for more work than the level it
    /// came from.
    ...
    /// The floor of a TIER OR SET BAND — the bottom of the mod-8 block the
    /// level sits in. Nothing lighter exists inside one variation, and the
    /// evaluative descent never steps past it.
    ///
    ...
    /// the set count and the starting dose (`repStartBand`) — `squat` 32 → 31
    /// reads as 4×4 → 3×11, 16 reps against 33. For levels 0..31 the block and
    /// the tier coincide word for word, and this equals `tierFloor`.
    static func bandFloor(_ level: Int) -> Int {
        let l = min(max(level, 0), EngineConfig.levelMax)
        return l - (l % EngineConfig.stepsPerTier)
    }

    /// The rung of a tier that carries a given rep dose — rep continuity. A
    /// descent into an easier variation keeps the NUMBER of reps, not the
    /// mod-8 rung: repStart grows down the tiers, so keeping the rung landed
    /// on the top of the lower tier with a higher dose.
    ...
    /// The encoding step of a unit — one rep, or as many seconds as ONE rung
    /// costs. The window of "the plan was met" is one step wide, so for reps
    /// it collapses to the old equality.
    ///
    /// For statics that step is no longer a constant. The ladder is relative,
    /// so a rung costs anywhere from 1 s (tier 4, bottom) to 4 s (tier 1,
    /// top), and the window has to equal ONE REAL rung — otherwise it drifts
    /// apart from the inversion again, exactly as in #139. The step is local:
    /// the interval to the right of the plan's rung, or on the last rung the
    /// interval to the left, there being no ladder further right.
    ...
    /// How much work the plan of a level asks for. Only comparable within one
    /// unit.
    ///
    /// Sides are IN the measure. The old wording — "sides are a property of
    /// the variation, not of the rung, so they stay out of it" — was the
    /// reason a descent from a two-sided movement onto a one-sided one passed
    /// the gate: hinge L24 3×4 with both legs → 3×5 per leg is 12 reps against
    /// 30. The measure takes a PAIR `(level, sub)`. `load` stays the BASE
    /// (smallest) dose of the plan, and the sub-steps' addition goes into
    /// `total`:
    ///
    ...
    /// The plan's sets are the band's LESS the ones taken off. The level fixes
    /// the variation and the dose per set; the cut only ever touches volume —
    /// neither the unit, nor the sides, nor the variation move — so the
    /// measure stays valid whatever the cut is: 3→2 is −33.3 %, 4→3 is −25.0
    /// %, 5→4 is −20.0 %.
    ///
    ...
    /// "no harder". A descent has no right to make the plan heavier — neither
    /// per set nor in total work across sides. An honest zero on a 4×4 band
    /// used to land on 3×8, half again as many reps of the same movement ("I
    /// said zero and it added more"). Same root cause as the landing above:
    ...
    /// in 41 the total work across sides grows. But fixing that here would
    /// declare a measure in reps valid across a change of variation, which it
    /// is not. The exemption stays; the pain path is closed by the first step
    /// of the descent, which never crosses a tier boundary at all.
    ///
    ...
    /// The nearest level at or below the inversion's result that does not ask
    /// for more work than the current plan. Applies to descents only — growth
    /// lives under the ceiling, where by construction nothing gets heavier.
    /// "the current plan" is the pair `(from, fromSub)`; the target of a
    /// descent always carries `sub == 0`, because every descent zeroes the
    /// sub-step. The descent reads the triple, and a landing that CHANGES THE
    /// UNIT is chosen by time under load rather than by the rung next door —
    /// see `landOnUnitChange`.
    static func descendNoHarder(pattern: Pattern, from: Int, factLevel: Int,
        ...
        if noHarder(pattern: pattern, from: from, to: factLevel,
            ...
            // The unit check stands on the EARLY return too — without it an
            // honest fact walked past it into the hang.
        ...
        // The loop covers ZERO as well. The old `while cand > 0` ended in an
        // unconditional `return 0` — a position the gate might itself have
        // rejected: 352 triples were handed out around the very check the gate
        // exists for.
    ...
    static func fromActual(pattern: Pattern, tier: Int, sets: Int, actual: Int) -> Int {
        ...
        // The inversion reads the same start and step the render used — the
        // band's own when the plan sits in a band.
        let repStart = EngineConfig.repStartBand[sets]
            ?? EngineConfig.repStart[tier] ?? EngineConfig.repMin
        let step: Int
        ...
        case .hold:
            // The static inversion is a lookup of the nearest rung ON THE
            // TABLE of the same ladder that drew the plan. A tie settles down,
            // and past the ladder's edge the edge interval carries on: the
            // caps and the gate apply on top.
