...
public enum EngineConfig {
    ...
    /// "less" is counted in SUB-STEPS. The same figure in
    ...
    /// How many "less" ratings in a row that named nothing
    ...
    /// 3 → 4. The reserve for the two blocks was spent
    ...
    /// Rest between sets by set band: 60 s was a constant
    ...
    /// 1–2 sets inherit the pause of a triple instead
    ...
    /// The technical ceiling of every counter and of the
    ...
    /// Rest reads the TIER as well as the band.
    ...
    /// The band starts drop 25/30 → 20/24. Tier 4 now tops
    ...
    /// bounds every other boundary lives in.
    ...
    /// The weekly ceiling on rises. The growth caps
    ...
    /// The floor on how much a single session ages the
    ...
    /// round to zero, the window never aged, and the weekly growth
    ...
    /// `setsFloorPain` is gone. It sat below the shared
    ...
    /// more. rejected a +50 % dose step as a breach of "do no harm",
    ...
    /// The chronic weak-link signal deferred
    /// earlier. The window counts a pattern's own APPEARANCES, not
    ...
    /// Sub-steps as well. The 2:1 ratio to a plain "less"
    /// is kept — both constants changed unit, neither changed magnitude.
    static let chronicStep = -2
    /// How many patterns calibrating from zero in
    ...
    /// The hold ladder — a relative step instead of a fixed
    /// five seconds.
    ///
    ...
    /// whole 3 s, so the ladder is derived from the start and the step
    ...
    /// are tier 4 by encoding. carries the table cell by cell with
    ...
    static let maxUpByPatternTier: [Pattern: [Int: Int]] = [
        ...
        // The cross-credit restores the pull slot's full
    ...
    /// Landing ceilings past the comeback table's edge:
    ...
    /// A ladder of tier BOTTOMS — every next storey of
    ...
    /// A "slow tissue" pattern is one the table holds to a step at
    /// EVERY tier — today only the calf. Derived from the table rather than
    /// kept as a second list: one source of truth.
...
public enum Engine {
    ...
    /// The gap in days is the engine's only numeric input
    /// besides the reported facts, and it sat outside the contract. In
    ...
    /// Where a single reported number puts the level.
    /// Pulled out of `applyFeedback` so the three rules read together —
    /// and so that function stays inside the lint's body-length ceiling.
    ///
    /// Answers a POSITION. Inside the "plan was met" window a
    ...
    private static func positionFromPointFact(pattern p: Pattern, exercise ex: SessionExercise,
        ...
        // The inversion reads the pattern's TRUE set band,
        // not the shown one. The gate clamps the PLAN, not the state —
        ...
        // "the plan was met" is a WINDOW, not a point.
        ...
        // reported fact across the whole static class (an earlier rule granted the
        // step only at equality). For reps the step is one and the window is
        // that old equality.
        // The window is one LOCAL rung of the ladder, not
        // five seconds, and it is read off the same band the inversion uses.
        // The window is measured from the plan's BASE dose —
        ...
        // The position is a TRIPLE, and the measure over
        ...
        // but the reps→level inversion is only valid one tier out (spec
        // ): the result is bounded by the neighboring tier's ceiling,
        // slow tissues by tier 1's. That ceiling bounds where a fact may LAND,
        // so it is a level and stays one; the landing carries a zero
        // sub-step.
        if oldL == 0 {
            ...
            // A calibration KEEPS the cut. It ignores the
            // ceiling on purpose — there is nothing to trust but the
        ...
        // "above or below" is settled by the DOSE, not by the
        ...
        if Level.ordinal(level: clampedFact, sub: 0) > Level.ordinal(level: oldL, sub: oldSub) {
            // The rise goes THROUGH `riseBy`, not by
            // an absolute landing that zeroes the cut. The old form handed
            // back every taken set at once and walked around ("no more
        ...
        // A descent may not make the plan heavier.
        // And it KEEPS the sets already taken off.
    ...
    /// Movements the user pointed at during the workout.
    /// The hold request left this list — the input was
    /// cancelled; so did the discomfort report. ONE named
    ...
    /// everyone — without that, someone for whom the whole plan is too
    ...
    private static func lessTargets(
        ...
        // The culprit is the pattern whose OWN appearances
        ...
        // of 62 failing appearances: the weak link is by
    ...
    ///
    /// The joint-pain input is GONE, and with it the
    ...
    ///
    /// Growth moves by SUB-STEPS. What used to be "+1 level"
    ...
    /// bounds a RISE counts sub-steps — the growth cell, the weekly
    /// window, the ramp — which is what keeps free for an honest
    ...
    /// Descents stay in WHOLE levels and zero the sub-step, so the
    /// guarantee does not stretch.
    ///
    /// The hold-this-level request was cancelled, and
    /// `gapDays` moved into its place as the seventh parameter. removes
    ...
    public static func applyFeedback(
        ...
        skipped: Set<Pattern> = [],
        /// The one aggregate the engine needs to stop
        /// daily training from multiplying its way around the growth caps.
        /// `nil` = the app supplies no signal, and the engine stays
        /// calendar-blind exactly as promises.
        /// FRACTIONAL days. Whole days threw away every
        /// gap shorter than one, so two workouts in a day froze growth for
        /// good; the app layer hands over `interval / 86_400`.
        gapDays: Double? = nil
    ) -> EngineState {
        // The state heals on entry, and a reported
        ...
        // The appearance window. Every exercise of the
        ...
        // Who calibrates from zero in THIS session.
        ...
        for ex in session.exercises {
            ...
            // The cell counts SUB-STEPS — a "2" means two
            ...
            // An exact fact does NOT fall under the
            // sub-step rule — the athlete's honesty is never overridden
            // Its path stays word for word: invert to a
            // level, pass the gate, zero the sub-step. Someone who wrote
            // "2 out of 8" is talking about a dose, not about fatigue, and
            // there is nothing to make finer.
            let factPath = overrides[p] != nil
            // The failure streak counts the INTENT to go
        ...
        Self.applyCrossCredit(&next, state: state, session: session, result: result,
                              overrides: overrides)
        // Remember what the person SAW and at which
        ...
        // The weekly ceiling is applied ONCE, after every
    ...
    /// at first, which is what made the rollout safe. The floor replaces
    ...
    /// double-counted.
    ///
    /// Split out of `applyFeedback` for the lint's function-length
    ...
    /// The weekly ceiling, applied ONCE per session over
    ...
    /// The budget is counted in SUB-STEPS. 's property
    ...
    private static func applyWeeklyCeiling(_ next: inout EngineState, state: EngineState,
        ...
        for p in Pattern.allCases {
            ...
            // The rise is measured on the SHARED scale — a
            ...
            // Rebuilding the position here used to walk
    ...
    /// A set came back — arm the hold; otherwise this
    ...
    /// The work a session showed, in the units of the
    ...
    /// Recording a shown plan WITHOUT any feedback.
    ...
    /// completed sessions.
    ///
    /// ACCEPTED until the app layer calls it: memory is written by a
    ...
    /// Writing a position. Sparseness is part of the
    ...
    static func setPosition(_ next: inout EngineState, _ p: Pattern, _ position: Position) {
        ...
        // A sub-step may not ask for more sets than the cut leaves.
        ...
        // Sparseness is part of the contract here too. What is
    ...
    /// The cross-credit on the pull slot. The slot is in
    ...
    /// to the other branch, capped by ITS OWN growth cell. Upward
    ...
    private static func applyCrossCredit(_ next: inout EngineState, state: EngineState,
        ...
        // The pause: a branch whose last appearance you called
        ...
        // branch is not in the plan, and a targeted "less" aims at
        ...
        // The credit never grows a frozen or sore
        // receiver — "a frozen pattern cannot grow" extends to
        ...
        // state's.
        // The gain is measured in SUB-STEPS and the receiver
        ...
        // handing the slot back the half speed the cross-credit was written to fix.
        // The gain is measured on the shared scale, or the
    ...
    /// Rolls the appearance window and returns the
    ...
    /// no fault of its own — which is exactly why the cross-credit gave the slot a
    /// cross-credit instead of its own feedback. The period-2 lock opened in
    /// must not latch again.
    ...
    /// The humble group landing. The group ceiling caps a
    /// from-zero calibration at the NEIGHBOUR tier's top — per pattern, and
    /// half the body can go there in one session while untangles it one
    ...
    private static func applyHumbleGroupLanding(_ next: inout EngineState,
        ...
        for p in calibratedUp {
            ...
            // The humble group landing KEEPS the cut
