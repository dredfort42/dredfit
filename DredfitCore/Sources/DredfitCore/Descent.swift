...
// with it. Split out of Engine.swift when the rating
//  path stopped being a one-line clamp and became a rule of its own.
//
// Until the rating was the last descent WITHOUT a gate: it went by
...
extension Engine {
    ...
    ///
    /// Up by SUB-STEPS.
    /// And down by them too. The evaluative descent steps
    /// one sub-step back along the growth path and never crosses the floor of
    /// its own block: until then it went by whole levels and landed in the
    /// MIDDLE of the tier below, where the dose is higher — `coreAntiExt`
    /// 24 → 23 meant 3×10 s → 3×31 s, `hinge` 24 → 23 meant 3×4 → 3×12 across
    /// both legs. The gate never saw it: the gate stands on the exact-fact
    /// path, and the rating path was the one descent without it.
    ...
    /// the failure streak: on a block floor the position does not move,
    ...
    /// Both directions now walk the SHARED scale —
    ...
    /// one otherwise, and `setsBackOk` is the hold on a returning set.
    ...
    /// because added two more of them and nine loose arguments is how a
    ...
    ///
    /// On the exact-fact path the streak reads the LEVEL,
    ...
    /// was not done.
    /// On the rating path the streak is fed by the INTENT
    ...
    /// variation, permanently out of reach. Holding is not an intent
    /// and still clears the streak.
    ///
    /// The deload passes the "no harder" gate for the
    ...
    /// their own number, so there it stays as it was.
    static func tickStreak(
        ...
        // The cut has to fit the NEW band. A deload
        ...
        // channel.
        // The floor here is the PAIN one. The shared
