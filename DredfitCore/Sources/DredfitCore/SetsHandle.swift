...
extension Level {
    ...
    /// The exercise's sets before the band gate. The floor is the shared
    /// one: the pain channel that once landed a movement on a single set is
    /// gone, and there is no second floor left to choose between.
    static func setsAfterCut(level: Int, cut: Int) -> Int {
        decode(level).sets - effCut(level: level, cut: cut,
                                    floor: EngineConfig.setsFloor)
    }

    /// The position's MEASURE. One growth event is exactly
    /// +1 and one step of a descent exactly −1, so the growth caps, the
    /// window and the cross-credit keep counting it as integers and stay
    /// the code they were.
    ///
    ...
    ///
    /// ACCEPTED PROPERTY: with a non-empty cut the scale is not
    ...
    /// exactly +1 (387 of 392, the rest being the scale's ceiling). The
    /// and ceilings bound GROWTH, and there the measure is exact; on a
    ...
    /// is ×1.500 median and ×2.00 worst), and rejected a +50 % dose step
    ...
    /// reverse of a growth event. On a block floor that path has run
    ...
    /// one place where a measure across the boundary stops being valid.
    static func fallBy(level: Int, sub: Int, cut: Int, by count: Int,
                       floor: Int) -> Position {
        // The same invariant on entry and at every step — a
    ...
    /// TIME UNDER LOAD — the only quantity
    /// that is comparable across a change of unit. Reps and seconds are
    /// incommensurable, but "how many seconds the muscle works" is
