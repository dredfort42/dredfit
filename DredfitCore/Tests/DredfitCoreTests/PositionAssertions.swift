...
extension XCTestCase {
    ...
    /// A position is a TRIPLE, so the third coordinate is
    /// compared too. Every earlier call site keeps reading as it did — a
    ...
    /// the growth cell, and standing still on a zero delta.
    /// DOWN in sub-steps too — one position back along the
    ...
    /// Both directions walk the shared scale — growth
    /// gives sets back first, a descent spends the dose before the sets — and
    /// the descent's floor is the pain one under a live episode.
    func expectedPosition(_ state: EngineState, _ p: DredfitCore.Pattern, delta: Int) -> Position {
        ...
        if delta < 0 {
            // ONE floor. The episode-aware exception went with
    ...
    /// Where a deload lands — `deloadDrop` levels below
    /// its base, pulled down by the "no harder" gate. The gate is the point:
    /// until the deload was a descent with no gate at all.
    ///
    /// The base differs by path. On the rating path it is the entry
    ...
    /// The deload reads the ACCUMULATED cut — it stands
    ...
    /// A step of the descent walks `fallBy` — the dose
