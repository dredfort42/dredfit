...
Проверять именно итог, а не код возврата пайпа: `... | tail` вернёт код `tail`, а не `xcodebuild`, и красный прогон выглядит зелёным. Смотреть на `** TEST SUCCEEDED **` и на строки `Executed N tests, with 0 failures`. Ожидаемо (1.4.0): 38 (ядро) + 50 (приложение, 1 skipped) + 24 (UI) = 112 тестов. Число растет с каждой волной — сверяйся с последним зеленым прогоном, а не с этой строкой.
