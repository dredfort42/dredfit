...
Проверять именно итог, а не код возврата пайпа: `... | tail` вернёт код `tail`, а не `xcodebuild`, и красный прогон выглядит зелёным. Смотреть на `** TEST SUCCEEDED **` и на строки `Executed N tests, with 0 failures`. Ожидаемо (1.6.0): 61 (ядро) + 88 (приложение) + 33 (UI) = 182 теста. Число растет с каждой волной — сверяйся с последним зеленым прогоном, а не с этой строкой.
