...
Three layers, 733 automated tests: 74 core + 579 app units + 80 UI tests, all confirmed green on this wave's run — core `[74/74]`, app units `Executed 579 tests, with 0 failures`, and UI a single `** TEST SUCCEEDED **` with no `Failing tests:` section and zero runner relaunches, so the closing tally belongs to the one session that ran (`Executed 80 tests, with 0 failures (0 unexpected) in 2965.3s` on the last full run, simulator erased before it).
