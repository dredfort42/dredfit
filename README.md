...
Three layers, 718 automated tests: 74 core + 566 app units + 78 UI tests, all confirmed green on this wave's run — core `[74/74]`, app units `Executed 566 tests, with 0 failures`, and UI a single `** TEST SUCCEEDED **` with no `Failing tests:` section and zero runner relaunches, so the closing tally belongs to the one session that ran (`Executed 77 tests, with 0 failures (0 unexpected) in 2795.7s`, simulator erased before the run).
