...
Three layers, 727 automated tests: 74 core + 575 app units + 78 UI tests, all confirmed green on this wave's run — core `[74/74]`, app units `Executed 575 tests, with 0 failures`, and UI a single `** TEST SUCCEEDED **` with no `Failing tests:` section and zero runner relaunches, so the closing tally belongs to the one session that ran (`Executed 78 tests, with 0 failures (0 unexpected) in 2901.1s`, simulator erased before the run).
