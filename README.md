...
steps**, stamped `adaptive_engine.js v3.4.0` — and the Swift port must reproduce
...
Three layers, 705 automated tests: 70 core + 558 app units + 77 UI tests, all confirmed green on this wave's run — core `[70/70]`, app units `Executed 558 tests, with 0 failures`, and UI a single `** TEST SUCCEEDED **` with no `Failing tests:` section and zero runner relaunches, so the closing tally belongs to the one session that ran (`Executed 77 tests, with 0 failures (0 unexpected) in 2795.7s`, simulator erased before the run).
...
Shipping. Working on device, tested across all seven locales. The newest tag is `v2.2.0`; `develop` carries 2.3.0, prepared and not yet released. The engine reference is 3.4.0, which 2.2.0 already shipped.
