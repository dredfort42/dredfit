...
Three layers, 596 automated tests: 70 core + 464 app units confirmed green on this wave's run, plus 62 UI tests by `func test` count — the UI suite's *executed* count is confirmed only when it runs in wave B3, so 596 is the declared total, not all of it a green-run total yet.
...
Shipping. Working on device, tested across all seven locales. The App Store carries 1.9.0; the engine on `develop` is 3.1.0, and the wave that ships it has not been released yet.
