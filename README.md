...
Three layers, 730 automated tests: 74 core + 576 app units + 80 UI tests, all confirmed green on this wave's run — core `[74/74]`, app units `Executed 576 tests, with 0 failures`, and UI a single `** TEST SUCCEEDED **` with no `Failing tests:` section and zero runner relaunches, so the closing tally belongs to the one session that ran (`Executed 80 tests, with 0 failures (0 unexpected) in 2950.2s` on the last full run, simulator erased before it).
