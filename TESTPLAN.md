...
| 50.12 | Let the LAST set of a plain hold reach zero | The clock does not stop. The done tone marks the plan, a ladder of steps appears, and every five seconds past the plan is banked when its own tone sounds. The big number is the seconds running, the button is the seconds banked, and the line under the ladder says why they differ |
| 50.13 | Stop the tail by hand | It records the BANKED figure — the last completed step — with no reach allowance taken off it: the rounding down already discarded the walk to the phone. The number is not marked as an estimate; it was earned in full |
| 50.14 | Let the tail run to its cap (twice the plan, never past 90 s) | It closes itself and records the cap, marked **≈**: nobody came back to the phone, so the app knows the hold lasted at least this long and must not claim to know more |
| 50.15 | Kill the app inside a tail and relaunch | **Continue the workout?** comes back INTO the running tail with the same bank, and the tones for steps already banked are not replayed. The tail is the one hold state that restores mid-count, and it has to be: the effort is still happening |
...
| 50.21 | VoiceOver on the summary and the tail | A card reads "set 2, approximately 48 seconds, planned 55" — one sentence with the comparison in it. The ladder reads "plan met, 5 seconds banked, next step at 65". The button reads "Stop, records 60 seconds" |
